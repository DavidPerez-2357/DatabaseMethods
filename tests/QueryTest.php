...
class QueryTest
{
    ...
    public function testFieldsWithStringNormalizesToArray()
    {
        $sql = Query::select()->from('t')->fields('id')->getQuery();
        assert_equals('SELECT id FROM t', $sql);
    }
    ...
    public function testJoinsWithArrayContainingNonStringThrows()
    {
        assert_throws('InvalidArgumentException', function () {
            Query::select()->from('t')->joins(['LEFT JOIN a ON a.id = t.a_id', 42]);
        });
    }

    public function testJoinsWithArrayContainingEmptyStringThrows()
    {
        assert_throws('InvalidArgumentException', function () {
            Query::select()->from('t')->joins(['LEFT JOIN a ON a.id = t.a_id', '']);
        });
    }
    ...
    public function testSelectOffsetWithoutLimitIsEmitted()
    {
        // OFFSET is unconditional — valid in e.g. PostgreSQL without LIMIT
        $sql = Query::select()->from('t')->offset(5)->getQuery();
        assert_contains('OFFSET 5', $sql);
    }

    public function testFieldsStringNormalizationOnSelectQuery()
    {
        $sql = Query::select()->from('t')->fields('name')->getQuery();
        assert_equals('SELECT name FROM t', $sql);
    }
