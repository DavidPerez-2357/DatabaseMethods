...
class DatabaseTest
{
    ...
    // =========================================================================
    // Tests - Query::run() via Database::createQuery()
    // =========================================================================

    public function testRunSelectReturnsAllRows()
    {
        $this->db->insert(self::TABLE, ['name' => 'Alice', 'email' => 'alice@example.com']);
        $this->db->insert(self::TABLE, ['name' => 'Bob',   'email' => 'bob@example.com']);
    ...
    public function testRunSelectWithWhereFiltersRows()
    {
        $this->db->insert(self::TABLE, ['name' => 'Charlie', 'email' => 'charlie@example.com']);
        $this->db->insert(self::TABLE, ['name' => 'Diana',   'email' => 'diana@example.com']);
        ...
        assert_equals(1, count($rows));
        assert_equals('Charlie', $rows[0]['name']);
    }

    public function testRunInsertSingleRowReturnsLastInsertId()
    {
        $id = $this->db->createQuery()
            ->insert(self::TABLE, ['name', 'email'])
            ->run(['name' => 'Eve', 'email' => 'eve@example.com']);

        assert_true(is_int($id));
        assert_true($id > 0);
    }

    public function testRunInsertSingleRowPersistsRecord()
    {
        $this->db->createQuery()
            ->insert(self::TABLE, ['name', 'email'])
            ->run(['name' => 'Frank', 'email' => 'frank@example.com']);

        $rows = $this->db->select(
            new Query(['method' => 'SELECT', 'fields' => ['name'], 'table' => self::TABLE, 'where' => "name = 'Frank'"])
        );
        assert_equals(1, count($rows));
        assert_equals('Frank', $rows[0]['name']);
    }

    public function testRunInsertMultiRowPersistsAllRecords()
    {
        $this->db->createQuery()
            ->insert(self::TABLE, ['name', 'email'])
            ->run([
        ...
        $rows = $this->db->select(
            new Query(['method' => 'SELECT', 'fields' => ['name'], 'table' => self::TABLE,
                       'where' => "name IN ('Grace', 'Hank')"])
        );
        assert_equals(2, count($rows));
    }

    public function testRunUpdateModifiesRecord()
    {
        $this->db->insert(self::TABLE, ['name' => 'Ivan', 'email' => 'ivan@example.com']);

        $affected = $this->db->createQuery()
            ->update(self::TABLE, ['name'])
            ->where('name = :old_name')
            ->run(['name' => 'Ivan Updated', 'old_name' => 'Ivan']);

        assert_equals(1, $affected);

        $rows = $this->db->select(
            new Query(['method' => 'SELECT', 'fields' => ['name'], 'table' => self::TABLE,
                       'where' => "name = 'Ivan Updated'"])
        );
        assert_equals(1, count($rows));
    }

    public function testRunUpdateReturnsZeroWhenNoMatch()
    {
        $affected = $this->db->createQuery()
            ->update(self::TABLE, ['name'])
            ->where('name = :old_name')
            ->run(['name' => 'NewName', 'old_name' => 'NonExistent']);

        assert_equals(0, $affected);
    }

    public function testRunDeleteRemovesRecord()
    {
        $this->db->insert(self::TABLE, ['name' => 'Judy', 'email' => 'judy@example.com']);
        ...
        assert_equals(0, count($rows));
    }

    public function testRunDeleteReturnsZeroWhenNoMatch()
    {
        $affected = $this->db->createQuery()
            ->delete(self::TABLE)
            ->where('name = :name')
            ->run([':name' => 'NonExistent']);

        assert_equals(0, $affected);
    }

    public function testRunThrowsWhenNoDatabaseLinked()
    {
        $caughtException = false;
        try {
            Query::select(['id'])->from(self::TABLE)->run();
        } catch (RuntimeException $e) {
            $caughtException = true;
        }
        assert_true($caughtException, 'run() must throw RuntimeException when no Database is linked.');
    }

    public function testRunThrowsWhenNoMethodSet()
    {
        $caughtException = false;
        try {
            $query = $this->db->createQuery();
        ...
        assert_true($caughtException, 'run() must throw InvalidArgumentException when method is not set.');
    }

    public function testSetDatabaseLinksQueryToDatabase()
    {
        $query = Query::select(['name'])->from(self::TABLE);
        $query->setDatabase($this->db);
        $result = $query->run();
        assert_true(is_array($result));
    }

    public function testCreateQueryReturnsBlankQueryWithDatabaseLink()
    {
        $query = $this->db->createQuery();
        assert_true($query instanceof Query);
        // A blank query must not have a method set — run() must throw on method-less query.
        $caughtException = false;
        try {
            $query->run();
        } catch (InvalidArgumentException $e) {
        ...
        assert_true($caughtException, 'createQuery() must return a blank (no-method) query.');
    }
}
