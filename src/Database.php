...
class Database
{
     ...
     * Delegates to update() for connection management, query building, and execution.
     *
     * WHERE placeholders use a 'where_' prefix (':where_{col}'), so they cannot collide
     * with the unprefixed SET placeholders (':{col}') that update() generates for normal
     * column names. update()'s normalizeNamedWhereBindings() will throw a clear error if
     * a data column is literally named 'where_{conditionColumn}'.
     *
     * Generated SQL:
     *   UPDATE table SET col1 = :col1 WHERE cond1 = :where_cond1
     *
     * @param string $table      Table name (must be a valid SQL identifier).
    ...
    public function updateWhere($table, array $data, array $conditions)
    {
        ...
        // Build WHERE clause using 'where_' prefix so that WHERE placeholders (':where_{col}')
        // cannot collide with the SET placeholders (':{col}') generated by update(). The only
        // theoretical collision would require a data column literally named 'where_{condCol}',
        // which update() catches via normalizeNamedWhereBindings() with a clear error.
