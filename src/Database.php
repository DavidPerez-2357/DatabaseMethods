...
class Database
{
    ...
    {
        return $this->plainSelect((string) $query, $data);
    }
     ...
     *
     * When `$data` is a sequential list of associative arrays, a multi-row insert is
     * performed. Otherwise `$data` is treated as a single-row associative array.
     *
     * If a field list was already set on the Query, the SQL is built from those fields
     * and `$data` values are mapped to the generated placeholders. When no field list is
     * pre-set, the fields are inferred from the keys of `$data` (single row) or the
     * first row (multi-row).
     *
    ...
    public function runInsert(Query $query, array $data = array())
    {
        ...
        if ($isMultiRow) {
            ...
            $this->runPlainQuery((string) $query, PdoParameterBuilder::buildInsertParams($rows));
            return 0;
        }
        ...
        $this->runPlainQuery((string) $query, PdoParameterBuilder::buildInsertParams(array($data)));
     ...
     *
     * `$data` must be a flat associative array containing both the SET values (keys
     * matching the Query's field list) and the WHERE bindings (all other keys).
    ...
    public function runUpdate(Query $query, array $data = array())
    {
        ...
        return (int) $this->runPlainQuery((string) $query, $placeholders);
    }
    ...
    {
        return (int) $this->runPlainQuery((string) $query, $data);
    }
